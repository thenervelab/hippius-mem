...
| I-WRAP-FRAME | `WrappedKey::signing_bytes` length-frames `epoch` and every other signed field. | `tampering_wrap_fields_breaks_the_signature` + wrap/unwrap proptest | test | write `epoch` as raw LE instead of `push_framed` | pairwise injectivity of wrap fields is still untested |
...
| I-REDACT-RETAIN | `retain` drops a redacted id even when its lamport exceeds the baseline. | `retain_drops_a_redacted_id_even_when_its_lamport_exceeds_baseline` | test | drop the `redacted` conjunct in `retain` | forget still uses the baseline escape (`retain_keeps_an_entry_newer_than_the_sync_baseline`) |
