...
| I-ANCHOR-SIGN | Every anchor record the store persists is signed over all its fields; a present-but-invalid signature is dropped as tamper on read; an unsigned record reads under `UnsignedAnchorPolicy::Accept` (the default) and is dropped-and-counted under `Reject` (`require_signed_anchors`). | `every_persisted_anchor_record_is_signed_by_its_author` | test | skip `sign_with` in `reserve_seq_and_persist` | per-field table is `signature_state_distinguishes_valid_unsigned_and_tampered`; tamper-skip + legacy-unsigned-accepted is `read_skips_a_tampered_signed_record_but_keeps_a_legacy_unsigned_one`; the strict arms are `reject_policy_drops_an_unsigned_record_and_counts_it` / `accept_policy_reads_unsigned_records_and_counts_them`, the closed residual end-to-end is `strict_mode_ignores_a_planted_fresh_unsigned_record` (e2e_durability) — the unsigned arm under the DEFAULT posture stays the documented migration residual in SECURITY.md |
...
| I-RECONCILE-HEAD | A returning machine reports a dropped or rolled-back author head as `head_regressions`; the watermark advances only after a head publish succeeds. | `a_returning_machine_reports_a_rolled_back_head`, `a_returning_machine_reports_a_dropped_head`; the advance-only-on-success clause is pinned by `a_failed_head_publish_does_not_advance_our_own_watermark` and `record_published_head_never_moves_a_mark_backward` | test | advance the mark before the publish succeeds (kills the failed-publish test), or drop the at-or-above guard (kills the never-backward test) | a fresh machine has no mark; an older but validly signed head is undetected — pinned as such by `a_stale_but_validly_signed_head_is_undetected` |
...
| I-SEAL-AAD | `open` refuses ciphertext presented under a different object key — the AAD folds the key into the tag, turning a gateway relocating note A's bytes to note B's key into an authentication failure. | `mismatched_aad_fails_auth` (the primitive), `relocated_object_fails_to_open` (the store call sites) | test | constant-AAD a store seal/open call site — kills the relocation test; the unit test pins only the primitive's refusal and survives a call-site mutation | a same-key byte swap is caught by the op-attested content hash, not by AAD |
...
| I-MCP-HTTP | The real binary over loopback streamable HTTP remembers a note, recalls it, and `get`s the body; a second HTTP session on the same process sees the write; missing bearer is 401; `serve` refuses a two-profile config; an unknown flag fails before the store boots. | `the_binary_remembers_recalls_and_gets_over_http`, `two_http_sessions_share_one_process_and_see_each_others_writes`, `http_mcp_rejects_a_missing_bearer_token`, `serve_refuses_a_multi_profile_config`, `serve_rejects_an_unknown_flag_before_booting` | test-all-features | drop the `/mcp` nest, skip the bearer layer, mint a store per session, skip `shared_daemon_profile`, or parse args after `boot_serve` | FastEmbedder boot remains untested (`semantic_embeddings = false`); Gemini/OpenClaw stay on stdio |
