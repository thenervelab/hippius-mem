...
| I-MCP-HTTP | The real binary over loopback streamable HTTP remembers a note, recalls it, and `get`s the body; a second HTTP session on the same process sees the write; missing bearer is 401. | `the_binary_remembers_recalls_and_gets_over_http`, `two_http_sessions_share_one_process_and_see_each_others_writes`, `http_mcp_rejects_a_missing_bearer_token` | test-all-features | drop the `/mcp` nest, skip the bearer layer, or mint a store per session | FastEmbedder boot remains untested (`semantic_embeddings = false`); Gemini/OpenClaw stay on stdio |
