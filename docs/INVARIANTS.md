...
| I-ANCHOR-SIGN | Every anchor record the store persists is signed over all its fields; a present-but-invalid signature is dropped as tamper on read. | `every_persisted_anchor_record_is_signed_by_its_author` | test | skip `sign_with` in `reserve_seq_and_persist` | per-field table is `signature_state_distinguishes_valid_unsigned_and_tampered`; tamper-skip + legacy-unsigned-accepted (phase 1) is `read_skips_a_tampered_signed_record_but_keeps_a_legacy_unsigned_one` — the unsigned arm is the documented migration residual in SECURITY.md |
