...
| `relevance_floor` | `HIPPIUS_MEM_RELEVANCE_FLOOR` | Override the minimum cosine at which a candidate counts as a match, in `[0.0, 1.0]`. Lower = looser; higher = stricter. Defaults to the model's calibrated floor (MiniLM `0.25`, bge-small `0.51`). |
...
| — | `HIPPIUS_MEM_MNEMONIC` | The developer's BIP-39 mnemonic. Env var only — **never stored in the config file**. 🔒 Secret. Gates every mnemonic-derived flow (the list is not exhaustive): the startup epoch-key bootstrap wherever a store boots (`serve`, `brief`, `report`, `upgrade`, …), member-key publish (`join`), and console minting (`mint-token`, `invite`). Founder flows **degrade** without it: `rotate` / `remove` warn and rotate with only the configured key-ring. See [Operating model](#operating-model). |
| — | `HIPPIUS_MEM_RECALL_WINDOW_SECS` | Read by the **hooks**, not the server: how long one `recall` holds the PreToolUse edit-gate open for the session (default 1800 s). Env var only. See [Teams § Using it day to day](TEAMS.md#using-it-day-to-day). |
| — | `HIPPIUS_MEM_HOOKS_BYPASS` | Read by the **hooks**, not the server: `1` bypasses the recall edit-gate for emergencies. Env var only. Use sparingly — the gate is the enforcement half of the recall discipline. |
...
# relevance_floor = 0.51                         # override the model's calibrated floor
```
...
[[teams]]
name = "acme"                      # also the note namespace (object-key prefix)
orgs = ["github.com/acme"]         # repos under this org route here
bucket = "acme-memory"
access_key_id = "AKID..."
secret = "<secret>"
team_key_hex = "…64 hex…"
...
- **`doctor [--offline]`** — validates a configured bundle and proves the encryption
  ...
  is present — checks the Hermes memory-provider plugin (including that
  `plugin.yaml` lists `system_prompt_block`), sidecar, and `memory.provider`.
  An unwired Hermes home fails the run (so `doctor --offline` is not a green
  light for a Hermes agent that skipped `install --agent hermes`, or that still
  has the 0.2.0 plugin yaml);
...
| `link` | Assert a directed link from one note to another by `id`, with an optional `rel` (`supersedes`/`contradicts`/`refines`/`duplicates`; omitted = plain link). A `supersedes`/`duplicates` target is **demoted** in `recall` (still returned, tagged) so the newer note wins. Appends a signed `Relate` op (`Link` for a plain link). Links are **grow-only** — there is no unlink op; remedy a mislink with a `supersedes` link or by `forget`ting the note. | `{ linked: true }`. |
...
> first use, and the session dies with the process or 30 minutes after launch
> (`SESSION_TTL`), whichever comes first — restart `hippius-mem dashboard` for a
> fresh URL (also how you open the UI in another browser, or after the TTL's
> sudden 401s). That plaintext never leaves your
