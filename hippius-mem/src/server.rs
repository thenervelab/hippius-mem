...
impl MemoryServer {
    ...
    #[tool(
        description = "Audit the team memory: reconcile the visible op-log against the anchored Merkle roots and against each author's own signed head pointer. Reports any op anchored but now missing from the bucket, any anchor record whose root disagrees with its leaves, any author whose ops did not form one hash chain on this read, and any author whose signed head names a chain tip this view does not contain. Returns { ok, checked_batches, total_anchored_ops, missing_ops, root_mismatches, quarantined_authors, suppressed_tails }; ok is false when ANY of those four vectors is non-empty, so read the vectors to tell which failed. SCOPE (the anchoring checks): only ops that were actually anchored are covered — an op dropped before its batch anchored leaves no commitment to check, indistinguishable from never having been written. In the default local mode both the op-log AND the anchor records live in the same untrusted bucket, so the anchoring checks detect accidental or partial op-log loss but NOT adversarial suppression — a bucket that drops an op together with its anchor record leaves nothing to reconcile against. The `chain` feature does NOT close that gap: it only detects a record the bucket kept but never actually committed on-chain (a forged-but-self-consistent root), not a record dropped together with its op, since it too only checks records the bucket still serves. SCOPE (quarantined_authors): this one needs no anchor record, so it can implicate an UNANCHORED op — each entry names an author whose ops the verified read could not link into one genesis-rooted chain, and how many ops it therefore dropped. It proves a break, never a cause: a forked or suppressed op, an object the bucket dropped for good, an object this read merely failed to fetch or did not see listed, and an honest writer's own cancelled-but-durable append are indistinguishable at author granularity. The two fetch/listing causes clear themselves on a later read, and a cancelled-but-durable append now usually clears itself too (the writer best-effort reclaims the orphaned op object right after the failed append) — but that reclaim can itself fail, and a hostile fork or a real deletion never clears on its own, so a persistently non-empty vector still needs investigation. It also cannot see an author suppressed WHOLE — with no ops there is no chain to break — nor a tail truncated cleanly at the end of a chain. SCOPE (suppressed_tails): this is the tail-truncation check the hash chain cannot perform, and the only evidence here that survives an author's TAIL op being dropped together with its anchor record (a MID-chain drop dangles the next op's prev_op_hash and surfaces as quarantined_authors instead) — each write publishes a signed head naming that author's current tip, and an entry means the author SIGNED a tip no visible op reproduces, so nobody but that author could have made the claim. It does not prove suppression: the op may merely have failed to fetch or not been listed on this read (which clears itself on a re-run), or it may have been quarantined by a chain break, in which case the same author appears in quarantined_authors too. Do NOT read that pair as a fork: a bucket dropping one MID-chain op quarantines everything after the gap including the tail, so it produces the pair every time, while a fork produces it only when the planted branch wins the tiebreak. The pair means the signed tip is among the ops this read quarantined -- a reason to look harder, not to stand down. It NARROWS tail truncation without closing it — an author whose head object the bucket also drops makes no claim at all, and an older but still-validly-signed head names a tip that IS visible, so both are silent here. Detecting those needs a locally-remembered high-water mark the bucket cannot rewrite, which reports a dropped or rolled-back head only on a machine that already saw the newer one; a machine that never saw it stays blind. So an empty suppressed_tails is not proof that no tail was truncated."
    )]
